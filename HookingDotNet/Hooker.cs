namespace HookingDotNet
{
    using System;
    using System.Reflection;
    using System.Runtime.InteropServices;

    unsafe class Hooker // E9 00 00 00 00
    {
        ...
        private void CreateJumper(byte* sources)
        {
            //int jumpIndex = this.AfterIndexOf(sources,
            //    30,
            //    0x48, 0x8B, 0xEC/* mov rbp,rsp */  // { 0x48, 0x83/*, 0x00, 0x00*/ }; /* sub rsp,0h */
            //);
            //if (jumpIndex < 0)
            //{
            //    throw new ArgumentOutOfRangeException("Unable to find the address of the function header to which the jumper widget can be installed.");
            //}
            if (*sources == '\xE8' || *sources == '\xE9')
            {
                if (this.mJmpMethodAsmCode == NativeMethods.NULL)
                {
                    this.mJmpMethodAsmCode = Marshal.AllocHGlobal(12);
                    this.AdjustProtectMemoryPermissions(this.mJmpMethodAsmCode);
                }
                byte* jmper = (byte*)this.mJmpMethodAsmCode;
                {
                    int rrva = *(int*)(1 + sources);
                    byte* rdst = sources + (5 + rrva);

                    if (sizeof(void*) <= sizeof(int))
                    {
                        jmper[0] = 0xE8;
                        long drva = (long)(5 + jmper);
                        *(int*)(1 + jmper) = (int)(rdst - drva);
                    }
                    else
                    {
                        jmper[0] = 0x48;
                        jmper[1] = 0xB8;
                        *(long*)&jmper[2] = (long)rdst;
                        jmper[10] = 0xFF;
                        jmper[11] = 0xD0;
                    }
                }
        ...
        public virtual IntPtr GetProcAddress(Delegate d)
        {
            if (d == null)
            {
                return IntPtr.Zero;
        ...
        public virtual IntPtr GetProcAddress(MethodBase m)
        {
            if (m == null)
            {
                return IntPtr.Zero;
